import ui, photos, console, numpy, Image
from scene import Rect, Point
...
class PixelEditor (ui.View):
	...
	def reset(self, row=None, column=None):
		...
		self.superview['preview'].image = self.image_view.image
...
class ColorView (ui.View):
	...
	def choose_color(self, sender):
		if sender.name in self.color.keys():
...
root_view = ui.load_view('pixel_editor')
root_view.present()
